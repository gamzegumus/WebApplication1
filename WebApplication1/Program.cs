...
IServiceCollection serviceCollection = builder.Services.AddDbContext<LoginFlowDBContext>(options => options.UseSqlServer(connstring));
...
{
    var Key = Encoding.UTF8.GetBytes(s: builder.Configuration["JWT:Key"]);
    k.SaveToken = true;
